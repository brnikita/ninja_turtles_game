...
// Scene (early-morning New York skyline)
const scene = new THREE.Scene();
scene.background = new THREE.Color(0x87ceeb); // Soft blue morning sky
scene.fog = new THREE.Fog(0xe0f6ff, 500, 2500); // Hazy dawn fog
...
// Renderer
const renderer = new THREE.WebGLRenderer({ antialias: false }); // Disable MSAA for performance
renderer.setSize(1000, 600);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // Cap pixel density on HiDPI screens
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = THREE.PCFShadowMap; // Faster shadows
container.insertBefore(renderer.domElement, document.getElementById('ui'));

// === LIGHTING ===
const ambientLight = new THREE.AmbientLight(0xffffff, 0.6); // Bright morning fill light
scene.add(ambientLight);

// Sunrise key light
const sunLight = new THREE.DirectionalLight(0xffddaa, 1.2); // Warm sunrise tone
sunLight.position.set(800, 300, -800); // Low sun angle
...
// Obstacle set used for collision checks
const obstacles = [];

// Helper to register an obstacle bounding box
...
// Collision test against all registered obstacles
...
const materials = {
    ...
    windowLight: new THREE.MeshBasicMaterial({ color: 0xffeeb0 }), // Lit apartment windows
    neonBlue: new THREE.MeshBasicMaterial({ color: 0x00ffff }),
    neonPink: new THREE.MeshBasicMaterial({ color: 0xff00ff }),
    copper: new THREE.MeshStandardMaterial({ color: 0x2c5a4c, roughness: 0.6 }), // Statue of Liberty copper
...
let steamParticles = []; // Declared before any steam emitters are created
...
function createCity() {
    const blockSize = 500; // Bigger city block footprint
    const streetWidth = 250; // Wider streets for better scale
    const mapSize = 2; // Fewer blocks, but much larger assets
    ...
    const windowGeo = new THREE.PlaneGeometry(15, 25); // Larger window tiles
    ...
    for (let x = -mapSize; x <= mapSize; x++) {
        for (let z = -mapSize; z <= mapSize; z++) {
            ...
            const sw = new THREE.Mesh(
                new THREE.BoxGeometry(swSize, 8, swSize), // Slightly elevated sidewalk
                materials.sidewalk
            );
            ...
            // Treat the sidewalk footprint as a no-go zone (protects buildings)
            addObstacle(xPos, zPos, swSize, swSize);

            // Towering skyscraper per block
            const buildingCount = 1; // Single huge structure per block
            
            const w = swSize * 0.9;
            const d = swSize * 0.9;
            const h = 300 + Math.random() * 600; // Up to ~900 units tall
            ...
            if (windowCount < maxWindows - 100) {
                ...
                for (let r = 0; r < rows; r++) {
                    ...
                    for(let c = 0; c < cols; c++) {
                        ...
                        // Randomize which facade the window belongs to
    ...
    // 3. Street props: lights, trees, cars, manholes
...
function createDecorations(blockSize, streetWidth, mapSize) {
    // Trees and lights along each street
    for (let x = -mapSize; x <= mapSize; x++) {
        for (let z = -mapSize; z <= mapSize; z++) {
            // Skip the arena in the center
            ...
            corners.forEach(pos => {
                ...
                addObstacle(pos[0], pos[1], 5, 5); // Treat the pole as solid

                // Tree near the pole
                createTree(pos[0] + (Math.random()-0.5)*30, pos[1] + (Math.random()-0.5)*30);
                
                // Random manhole on the road
                if (Math.random() > 0.7) {
            ...
            if (Math.random() > 0.4) {
                ...
                addObstacle(carX, carZ, 30, 60); // Cars are solid obstacles
            }
        }
    }
    
    // Extra center details
    createManhole(50, 50);
    createManhole(-80, -120);
}

// Build the city as soon as helper functions are ready
createCity();
...
function createStreetLights() {
    // Legacy helper (kept for reference, not used directly)
    ...
    positions.forEach(pos => {
        ...
        // Lamp
        const light = new THREE.SpotLight(0xffaa00, 0.8, 150, Math.PI/3, 0.5, 1); // Reduced range and cone
        light.position.set(pos[0], poleHeight, pos[1]);
        light.target.position.set(pos[0], 0, pos[1]);
        light.castShadow = false; // Optimization: no shadows from street lights
        scene.add(light);
...
function createStatueOfLiberty(x, z) {
    ...
    // Scale up to sell the distance
    statueGroup.scale.set(3, 3, 3);

    scene.add(statueGroup);
}

// Steam helper referenced by arena elements
...
// Request pointer lock when the player clicks the canvas
...
// Mouse movement updates the orbit angles
document.addEventListener('mousemove', (e) => {
    if (isPointerLocked) {
        mouseX += e.movementX * 0.002;
        mouseY += e.movementY * 0.002;
        mouseY = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, mouseY)); // Clamp vertical orbit
    }
});

// Mouse attack shortcut
document.addEventListener('mousedown', (e) => {
    if (e.button === 0 && isPointerLocked) { // Left mouse button
        keys.space = true;
    }
});
...
class Player {
    constructor() {
        ...
        // --- BODY ---
        // Lift the torso so it sits on top of the legs
        const torsoGeo = new THREE.BoxGeometry(32, 40, 18);
        const torso = new THREE.Mesh(torsoGeo, materials.skin);
        torso.position.y = 50; // Previously 20; now aligned with tall legs
        ...
        // Red scar through the right eye
        ...
        // Heavy shoulder pad on the left side
        ...
        strap.rotation.z = -0.6; // Diagonal belt
        strap.scale.z = 1.1;
        this.mesh.add(strap);
        ...
        // Legs (oversized so they are clearly visible)
        const legGeo = new THREE.BoxGeometry(14, 30, 16);
        
        // Group wrapper so walk animation can offset legs independently
        ...
        this.leftLeg.position.set(-10, 15, 0); // Half the height to rest on ground
        ...
        // Torso and leg heights are aligned so their bottoms sit on y=0
    ...
    update() {
        ...
        // Keep player inside the enlarged map
        ...
        // Wiggle mask tails for some life
...
class Enemy {
    constructor(x, z) {
        ...
        symbol.rotation.z = Math.PI; // Upside-down triangle
...
class Boss {
    ...
    update() {
        ...
        // Dash (fast but still respects collisions)
    ...
    takeDamage(amount) {
        this.health -= amount;
        // Quick wireframe flicker to signal hit
        this.mesh.children.forEach(c => {
        ...
        if (this.health <= 0) {
            ...
            score += 1000;
            updateScore();
            createExplosion(this.x, 40, this.z, 0xffaa00, 50); // Large explosion
        }
    }
}
...
function spawnEnemy() {
    const side = Math.floor(Math.random() * 4);
    let x, z;
    const dist = 800; // Spawn slightly farther out but still reachable
    
    // Try up to 10 times to find a spawn spot outside collisions
    let attempts = 0;
    do {
        switch(side) {
...
function checkAttackHit(attacker) {
    // Katana hitbox projected from player forward vector
...
function animate() {
    ...
    // Camera follows the player but orbits using mouse input
    const camDistance = 400;
    const camHeight = 200;
    
    // Position camera using yaw/pitch derived from mouse deltas
